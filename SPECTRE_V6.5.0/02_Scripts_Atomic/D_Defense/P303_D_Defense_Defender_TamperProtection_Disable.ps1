<#
    ID: 303 | Name: Defender_TamperProtection_Disable
    Artifact de comportement : V4.9.3 Lock
    Description : Neutralisation de la Tamper Protection (Necessite souvent un boot hors-ligne ou un token TrustedInstaller)
#>
[CmdletBinding()]
...
$Context = Get-SpectrePointRef -PointID "303"

Write-Host "`n[ATOME 303] Defender_TamperProtection_Disable" -ForegroundColor Cyan
...
if ($Analyse) {
    ...
    return [PSCustomObject]@{ID="303"; Name="Defender_TamperProtection_Disable"; Status="ANALYSE_$Status"; Updated=$false}
}
