<#
.DESCRIPTION
    NOM : 02_Action_Commit.ps1 | TYPE : Actionneur
...
# --- 2. EXECUTION ---
Write-Host "--- [SPECTRE] ACTION : COMMIT ---" -ForegroundColor Yellow
$Atoms = Get-ChildItem $AtomsDir -Filter "$Scope*.ps1" | Sort-Object Name

foreach ($Atom in $Atoms) {
    Write-Host "`n[!] Cible : $($Atom.Name)" -ForegroundColor Red
    & $Atom.FullName -Mode Commit
}
Write-Host "`n--- [FIN D'ACTION] ---" -ForegroundColor Yellow
