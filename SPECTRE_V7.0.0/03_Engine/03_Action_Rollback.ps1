<#
.DESCRIPTION
    NOM : 03_Action_Rollback.ps1 | TYPE : Actionneur
...
# --- 2. EXECUTION ---
Write-Host "--- [SPECTRE] ACTION : ROLLBACK ---" -ForegroundColor Green
$Atoms = Get-ChildItem $AtomsDir -Filter "$Scope*.ps1" | Sort-Object Name

foreach ($Atom in $Atoms) {
    Write-Host "`n[R] Cible : $($Atom.Name)" -ForegroundColor Green
    & $Atom.FullName -Mode Rollback
}
Write-Host "`n--- [FIN D'ACTION] ---" -ForegroundColor Green
